    ...
    distance_sum_(0.0), angle_sum_(0.0), max_index_(0) 
{
    ...
    map_sub_ = nh_.subscribe(map_topic_name_,1,&ParentMCL::map_callback,this);
    scan_sub_ = nh_.subscribe(scan_topic_name_,1,&ParentMCL::scan_callback,this);
    obj_sub_ = nh_.subscribe(obj_topic_name_,1,&ParentMCL::obj_callback,this);
    timer_ = nh_.createTimer(ros::Duration(1),&ParentMCL::timer_callback,this);
...
ParentMCL::Particle::Particle(ParentMCL* mcl) : m_(mcl)
{
...
ParentMCL::Particle::~Particle() { }

void ParentMCL::Particle::init(double x,double y,double yaw,double x_cov,double y_cov,double yaw_cov)
{
    do{
...
void ParentMCL::Particle::motion_update()
{
...
void ParentMCL::Particle::move(double dx,double dy,double dyaw)
{
    double delta_rot1;
...
void ParentMCL::Particle::measurement_update()
{
    double angle;
    double map_range;
...
void ParentMCL::map_callback(const nav_msgs::OccupancyGridConstPtr& msg)
{
    map_ = *msg;
...
void ParentMCL::scan_callback(const sensor_msgs::LaserScanConstPtr& msg) { scan_ = *msg; }
...
void ParentMCL::timer_callback(const ros::TimerEvent& event)
{
...
void ParentMCL::set_pose(geometry_msgs::PoseStamped& pose,double x,double y,double yaw)
{
...
void ParentMCL::spread_particles()
{
...
void ParentMCL::measurement_process()
{
    likelihood_sum_ = 0.0;
...
void ParentMCL::calc_likelihood()
{
    max_index_ = 0;
    for(int i = 0; i < NUM_OF_PARTICLES_; i++){
...
void ParentMCL::publish_est_pose()
{
...
int ParentMCL::map_grid_data(double x,double y)
{
...
double ParentMCL::calc_yaw_from_quat(geometry_msgs::Quaternion q)
{
...
double ParentMCL::calc_angle_diff(double a,double b)
{
...
double ParentMCL::calc_range(double x,double y,double yaw)
{
